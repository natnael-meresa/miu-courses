...
public class MyStringList {
	...
	public int indexOf(String s) {
		if (isEmpty()) return  -1;
		for (int i = 0; i < size; i++) {
			if (Objects.equals(strArray[i], s)) {
				return i;
			}
		}
	...
	public boolean containsAll(MyStringList others) {
		if (isEmpty() || others == null || others.isEmpty()) return false;
		for (int i = 0; i < others.size; i++) {
			boolean found = false;
			for (int j = 0; j < size; j++) {
				if (Objects.equals(strArray[j], others.strArray[i])) {
					found = true;
					break;
				}
			}
	...
	public MyStringList subList(int start, int end) {
		...
		MyStringList newMyList = new MyStringList();
		int subSize = end - start;

        System.arraycopy(strArray, start, newMyList.strArray, start - subSize, end - start);
		newMyList.size = subSize;
